...
@section('container')
  <div class=" overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <div class="flex items-center gap-2 mb-2 flex-wrap">
      @hasrole(['Pendamping', 'Super Admin'])
        <a href="/izinPulangPendamping/create" class=" bg-violet-500 text-white px-3 py-1 rounded-md inline-block">Tambah</a>
      @endhasrole
            ...
            <td class="px-6 py-4">
              @if (!request()->is('keamanan*'))
                @if ($izin->user_id == auth()->user()->id)
                  <a href="/izinPulangPendamping/{{ $izin->id }}/edit"
                    class="text-white bg-sky-500 rounded-lg text-xs px-3 py-1 inline-block">Edit</a>
                @endif
              @endif

              @hasrole(['Pengasuh', 'Super Admin'])
                @if ($izin->persetujuan_pengasuh == 'Belum')
                  <button type="button"
                    class="togglePersetujuanPengasuhModal mt-2 text-white bg-green-500 rounded-lg text-xs px-3 py-1 inline-block cursor-pointer"
                    data-modal-target="popup-modal-persetujuan-pengasuh"
                    data-modal-toggle="popup-modal-persetujuan-pengasuh"
                    data-title="Yakin Menyetujui Izin {{ $izin->user->name }}?" data-data="{{ $izin }}"
                    data-setujui="Ya" data-url="izinPulangPendamping/persetujuanPengasuh">Setujui</button>

                  <button type="button"
                    class="togglePersetujuanPengasuhModal mt-2 text-white bg-red-500 rounded-lg text-xs px-3 py-1 inline-block cursor-pointer"
                    data-modal-target="popup-modal-persetujuan-pengasuh"
                    data-modal-toggle="popup-modal-persetujuan-pengasuh"
                    data-title="Yakin Menolak Izin {{ $izin->user->name }}?" data-data="{{ $izin }}"
                    data-setujui="Tidak" data-url="izinPulangPendamping/persetujuanPengasuh">Tolak</button>
                @endif
              @endhasrole
              ...
              @hasrole(['Keamanan', 'Super Admin'])
                @if ($izin->cek_satpam == false)
                  <button type="button"
                    ...
                    data-url="izinPulangPendamping/cekSatpam">Cek</button>
                @endif
              @endhasrole
              ...
              @if ($izin->waktu_kembali == null)
                <button type="button"
                  ...
                  data-url="izinPulangPendamping/kembali">Kembali</button>
              @endif
    ...
    @include('partials.popUpPersetujuanPengasuh')
  </div>
@endsection
