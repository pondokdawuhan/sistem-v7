          ...
          @foreach ($izins as $izin)
            <tr
              ...
              <td class="px-3 py-1"><a wire:navigate
                  href="/{{ $lembaga->id ?? 'semua' }}/{{ $role }}/detailIzinKeluarSantri/{{ $izin->santri->username }}">{{ $izin->santri->name }}</a>
              </td>
              ...
              <td class="px-3 py-1">
                @if ($role != 'keamanan')
                  @if (strtotime($izin->waktu_selesai) > time() && $izin->waktu_kembali == null)
                    @if ($izin->user_id == auth()->user()->id)
                      <a wire:navigate
                        href="/{{ $lembaga->id }}/{{ $role }}/izinKeluarSantri/{{ $izin->id }}/edit"
                ...
                @if ($role != 'keamanan')
                  @if ($izin->user_id == auth()->user()->id && $izin->waktu_kembali == null)
                    <button type="button"
