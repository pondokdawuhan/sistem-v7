@if ($santris)
  <form action="/{{ $lembaga->id }}/nilaiSantri" method="POST">
    @csrf
    <div class="">
      <div class="text-center text-slate-900 dark:text-white">
        <h3>Tambah Nilai Kelas {{ $kelas->nama }} Pelajaran {{ $pelajaran->nama }}</h3>
        <h3>Semester {{ $selectedSemester }} Tahun
          Pelajaran {{ $selectedTahun }}</h3>
      </div>
      <div class="">
        <input type="hidden" name="pelajaran" value="{{ $selectedPelajaran }}">
