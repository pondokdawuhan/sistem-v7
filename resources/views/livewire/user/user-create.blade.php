<div>
  <x-loading></x-loading>
  <form method="POST" enctype="multipart/form-data" wire:submit.prevent='tambah'>
    @csrf
            ...
            <div class="w-full lg:w-1/5">
              ...
              @foreach ($lembaga->kelas as $kelas)
                <div class="flex items-center dark:text-white gap-2 mt-2">
                  <div class="block min-h-6 pl-2">
                    <label>
                      <input id="{{ $kelas->nama }}{{ $kelas->id }}" class="checkbox bg-white checkbox-success"
                        type="checkbox" / wire:model="selectedKelas" value="{{ $kelas->id }}">
      ...
      document.addEventListener('livewire:init', () => {
        ...
        fetch(`https://bustanulmutaallimin.github.io/api-wilayah-indonesia/api/provinces.json`)
          ...
          .then(provinces => provinces.forEach(province => {
            inputProvinsi.innerHTML += `<option value="` + province.name + `" data-provinsi="` + province.id +
              `" class="text-slate-700 dark:text-white text-xs md:text-base">` + province.name + `</option>`
          }));
